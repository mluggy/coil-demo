...
root.push(`- Use the [MCP server](${SITE}/mcp) (Streamable HTTP, JSON-RPC 2.0) for native MCP clients — tools: \`search_episodes\`, \`get_episode\`, \`get_latest_episode\`. Supports JSON-RPC 2.0 batch (array of up to 50 requests).`);
...
api.push(`curl -i -X POST '${SITE}/ask?async=1' \\`);
api.push(`  -H 'Content-Type: application/json' \\`);
api.push(`  -d '{"query":"ai agents"}'`);
api.push(`# HTTP/1.1 202 Accepted`);
api.push(`# Location: ${SITE}/jobs/<id>`);
api.push(`# Retry-After: 1`);
api.push(`# { "job_id": "<id>", "status": "pending", "poll_url": "${SITE}/jobs/<id>", "retry_after_seconds": 1 }`);
api.push("");
api.push(`# 2. Poll the job — 200 OK`);
api.push(`curl ${SITE}/jobs/<id>`);
api.push(`# { "job_id": "<id>", "status": "completed", "result": { "query": "...", "results": [...] } }`);
api.push("```");
api.push("");
api.push("Job ids are stateless: the id encodes the spec, so any client that holds an id can resume polling later from a different IP / device.");
api.push("");
api.push(`### Status`);
...
api.push("");
api.push("Methods: `initialize`, `ping`, `tools/list`, `tools/call`, `resources/list`, `resources/read`.");
api.push("Tools: `search_episodes`, `get_episode`, `get_latest_episode`.");
api.push("Batch / bulk: the request body may be a JSON-RPC 2.0 batch — an array of up to 50 request objects run in one round-trip, answered by an array of responses in order.");
api.push("");
